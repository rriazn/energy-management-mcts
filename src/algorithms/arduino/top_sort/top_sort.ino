...
void generate_graph_and_topological_sort(void) {
    ...
    if (!root) {
        Serial.println("ERROR: Could not allocate root");
        return;
    }
...
void find_best_path_topological(void) {
    if (topo_count == 0) {
        Serial.println("No topological order");
        return;
    }
...
void setup() {
    ...
    Serial.println("Starting topological sort search...");
