...
static const int E[] = {2, 0, 0, 0, 0, 0, 0, 0, 0, 2, 6, 9, 10, 9, 6};
...
static int reconstruct_path(int battery_level, int path[K]) {
    ...
    for(int i = K - 1; i >= 0; i--) {
        ...
        quality += pgm_read_word(&tasks[path[i] - 2].quality);
        battery_level = MIN(B_MAX, battery_level + pgm_read_word(&tasks[path[i] - 2].cost) - pgm_read_word(&E[i]));
    }
    return quality;
}
...
static int a_star(int path[K]) {
    ...
    while (!pq_empty()) {
        ...
        for (int i = 0; i < NUM_TASKS; i++) {
            task_t task;
            memcpy_P(&task, &tasks[i], sizeof(task_t));
            
            int new_battery = MIN(B_MAX, current.battery + pgm_read_word(&E[current.timeslot]) - task.cost);
            
            if (new_battery < B_MIN) {
                continue;
            }
            
            int new_quality = current.quality + task.quality;
            ...
            best_parent[next_t - 1][new_battery - B_MIN] = task.id + 1;
...
void setup() {
    ...
    //Serial.println(F("Starting A* optimization..."));
